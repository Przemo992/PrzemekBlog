 ...
 * Article
 * 
 * A piece of writing for publocation
 * 
 */
 ...
 * @param integer $offset Numbet of records to skip
 * 
 * @return array An associative array of the page of article records
 */
public static function getPage($connect, $limit, $offset, $only_published = false){
 ...
 * 
 * @param object $connect connecion to the database
 ...
 */
 ...
 public static function getByID($connect, $id, $columns = '*'){
   ...
   * 
   * @param object $connect Connection to hte database
   * @param integer $id the article ID
   * 
   * @return array The article data with categories
   * 
   */
  public static function getWithCategories($connect, $id, $only_published = false){
 ...
 */
   ...
   * 
   * @param object $connect Connectipon to hte database
   * @param array $ids Category IDs
   * 
   * @return void
   */
  public function setCategories($connect, $ids){
...
protected function validate(){
  ...
  if ($this->published_at != ''){
    $date_time = date_create_from_format('Y-m-d\T G:i', $this->published_at);

    if ($date_time === false){
      $this->errors['published_at'] = 'Invalid date and time';
 ...
 */
 public function delete($connect){
   ...
   */
  public static function getTotal($connect, $only_published = false){

    $condition = $only_published ? ' WHERE published_at IS NOT NULL' : '';

    return $connect->query("SELECT COUNT(*) FROM article$condition")->fetchColumn();
   ...
   * 
   * @param object $connect Connection to the database
   * @param strinf $filename The filename of image file
   * 
   * @return boolean true if it was succesfull, false otherwise
   */

  public function setImageFile($connect, $filename){
  ...
  public function publish($connect){
